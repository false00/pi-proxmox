...
export function vmCreate(client) {
  return {
    ...
    description: "Creates a new QEMU/KVM virtual machine on a Proxmox node. Cloud-init credentials are passed via ciuser/cipassword. SSH keys can be pre-seeded via sshkeys. Use ide2 to attach an installation ISO or cloud image, and scsi0 to attach a disk backend (e.g. 'local-lvm:32').",
    parameters: Type.Object({
      ...
      ide2: Type.Optional(Type.String({ description: "CDROM or cloud image, e.g. 'local:iso/ubuntu.iso,media=cdrom'" })),
      scsi0: Type.Optional(Type.String({ description: "Boot disk, e.g. 'local-lvm:32'" })),
      boot: Type.Optional(Type.String({ description: "Boot order, e.g. 'order=scsi0;ide2'" })),
      ...
      ciuser: Type.Optional(Type.String({ description: "Cloud-init user (default: root)" })),
      cipassword: Type.Optional(Type.String({ description: "Cloud-init password" })),
      sshkeys: Type.Optional(Type.String({ description: "Cloud-init SSH public keys (newline-separated)" })),
